          ...
          <form className="space-y-6" onSubmit={form.handleSubmit((values) => mutation.mutate({ values, contract: editingContract }))}>
            ...
            <section className="rounded-2xl border border-slate-200 bg-slate-50/60 p-4">
              ...
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
                ...
                <label className="form-field"><span>Valor da caucao</span><input className="input" type="number" min="0" step="0.01" readOnly={Boolean(editingContract?.deposit_amount)} {...form.register('deposit_amount', { valueAsNumber: true })} /></label>
                <label className="form-field"><span>Data de pagamento da caucao</span><input className="input" type="date" disabled={Number(watched.deposit_amount) <= 0} readOnly={Boolean(editingContract?.deposit_amount)} {...form.register('deposit_paid_at')} /></label>
                <label className="form-field"><span>Forma de pagamento da caucao</span><select className="input" disabled={Number(watched.deposit_amount) <= 0} {...form.register('deposit_payment_method')}><option value="">Selecione</option><option value="pix">Pix</option><option value="card">Cartao</option><option value="transfer">Transferencia</option><option value="cash">Dinheiro</option><option value="other">Outro</option></select></label>
