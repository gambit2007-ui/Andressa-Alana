...
update public.device_sales sale
set buyer_name = coalesce(
  (select client.full_name from public.rental_clients client where client.id = sale.client_id),
  'Comprador avulso'
)
where buyer_name is null or btrim(buyer_name) = '';

alter table public.device_sales
  alter column buyer_name set default 'Comprador avulso',
  alter column buyer_name set not null;

alter table public.device_sales
  drop constraint if exists device_sales_buyer_name_check;
alter table public.device_sales
  add constraint device_sales_buyer_name_check
  check (char_length(btrim(buyer_name)) between 1 and 160);

drop function if exists public.create_direct_device_sale(
  uuid, uuid, uuid, numeric, timestamptz, text, text, boolean, text
);
...
begin
  ...
  if p_sale_amount is null or p_sale_amount <= 0
    ...
    or char_length(coalesce(p_buyer_name, '')) > 160 then
  ...
  if p_client_id is not null then
    ...
    client_name := nullif(btrim(coalesce(p_buyer_name, '')), '');
    if client_name is null then
      raise exception 'Informe o nome do comprador avulso';
    end if;
  end if;
  ...
  insert into public.device_sales(
    id, organization_id, device_id, client_id, buyer_name, sale_amount, sold_at,
    payment_method, paid_in_full, apple_release_confirmed, confirmed_by, notes
  ) values (
    sale_id_value, p_organization_id, p_device_id, p_client_id, client_name, p_sale_amount, p_sold_at,
...
revoke all on function public.create_direct_device_sale(
  uuid, uuid, uuid, numeric, timestamptz, text, text, boolean, text, text
) from public;
grant execute on function public.create_direct_device_sale(
  uuid, uuid, uuid, numeric, timestamptz, text, text, boolean, text, text
) to authenticated;
